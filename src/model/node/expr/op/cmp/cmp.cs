...
internal abstract class CmpOp:Op.Handler {

  public override string name => "hotc";

  bool valid(Type t) {
    if (t is types.Int) return true;
    if (t is types.Bool) return true;
    if (t is types.Float) return true;
    if (t is types.Enum) return true;
    return false;
  }
...
internal abstract class Cmp:OpExpr {

  public Cmp(Place place) : base(place) {}
